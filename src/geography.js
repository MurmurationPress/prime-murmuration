...
export class GeographyField {
  constructor(width,height,config,cols=90,rows=160) {
    ...
    for(let y=0;y<rows;y++)for(let x=0;x<cols;x++){
      const px=(x+.5)/cols*width,py=(y+.5)/rows*height,ll=unproject(px,py,width,height);
      this.values[y*cols+x]=preferenceAt(px,py,width,height,config);this.supports[y*cols+x]=habitatSupport(ll.lon,ll.lat);
    }
  }
  sampleSupport(x,y) {
